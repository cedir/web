...
class UpdateImportesYPagoContraFacturaTests(TestCase):
    ...
    def setUp(self):
        ...
        self.client = Client(HTTP_POST='localhost')
        self.client.login(username='walter', password='xx11')
        self.url = '/api/estudio/{}/update_importes_y_pago_contra_factura/'
...
class RetreiveEstudiosTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client.login(username='walter', password='xx11')
        self.estudio = Estudio.objects.create(fecha=datetime.today().date(), paciente_id=1, practica_id=1, medico_id=1,
                                              obra_social_id=1, medico_solicitante_id=1, anestesista_id=1, institucion=1)
    ...
    def test_filtro_por_institucion_funciona_si_institucion_es_cedir(self):
        results = Estudio.objects.all()

        response = json.loads(self.client.get('/api/estudio/?institucion={}'.format(ID_INSTITUCION_CEDIR), content_type='application/json').content)['results']

        for estudio in response:
            self.assertEqual(results[estudio['id'] - 1].institucion, ID_INSTITUCION_CEDIR)

    def test_filtro_por_institucion_funciona_si_institucion_es_hospital_italiano(self):
        results = Estudio.objects.all()

        response = json.loads(self.client.get('/api/estudio/?institucion={}'.format(ID_INSTITUCION_HOSPITAL_ITALIANO), content_type='application/json').content)['results']

        for estudio in response:
            self.assertEqual(results[estudio['id'] - 1].institucion, ID_INSTITUCION_HOSPITAL_ITALIANO)

    def test_filtro_por_institucion_devuelve_estudios_cedir_si_no_se_envia_institucion(self):
        results = Estudio.objects.all()
