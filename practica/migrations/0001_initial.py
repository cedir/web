# -*- coding: utf-8 -*-
# Generated by Django 1.10.1 on 2016-09-29 16:04


from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Practica',
            fields=[
                ('id', models.AutoField(db_column='idEstudio', primary_key=True, serialize=False)),
                ('descripcion', models.CharField(db_column='estudio', max_length=200, verbose_name='Descripcion')),
                ('codigoMedico', models.CharField(max_length=50)),
                ('abreviatura', models.CharField(max_length=20)),
                ('usedLevel', models.IntegerField()),
            ],
            options={
                'ordering': ['descripcion'],
                'db_table': 'AlmacenEstudios',
            },
        ),
    ]
