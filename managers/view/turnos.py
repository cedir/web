...
class ViewTurnos():
  ...
  def getDayLines(self,dayLines):
    ...
    for dayLine in dayLines:
	  ...
	  strMedico = ''
	  for char in medicoChars:strMedico += char + " "
      ...
      dayLineContext = Context({'lineDay':dayLine["dia"],'fechaTurno':dayLine["fecha"],'turnos':arrHshTurnos,'disponibilidades':arrHshDisponibilidad,})
      if strLines: strLines = strLines + dayLineTemplate.render(dayLineContext)
      else: strLines = dayLineTemplate.render(dayLineContext)
